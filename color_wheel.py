...
class color_wheel():
    ...
    def draw_wheel(self,step = 6,shades = 12,luminocity_value = 1,saturation_value = 1):
        ...
        #to make all pixels present, because inner wedges are smaller than outer and some inner rgb values
        #are overwritten
        if step<12:
            im = Image.new("RGB", (300,300),(255,255,255))
        else:
            im = Image.new("RGB", (24*step,24*step),(255,255,255))
        ...
        #create dictionary with luminocity ranges
        lum_dict = {}
        if self.luminocity == True:
            ...
            if self.grey_wheel==False:
                ...
                lum_range = 1,0
            lum_vals = np.round(np.arange(lum_range[0],lum_range[1]+(lum_range[1]-lum_range[0])/(step-1),
                                          (lum_range[1]-lum_range[0])/(step-1)),4)
        ...
        if self.saturation == True:
            step_length = radius/step
            sat_range = 1, 0.0
            sat_vals = np.round(np.arange(sat_range[0],sat_range[1]+(sat_range[1]-sat_range[0])/(step-1),
                                          (sat_range[1]-sat_range[0])/(step-1)),4)
            
            sat_vals = reversed(sat_vals)
            start=0
            for sat in sat_vals:
